...
int getIncarnationNumber(int failure) {
	...
	if (fp != NULL) {
		...
		/* Check for Client Failure */
		if (failure == TRUE) {
			/* Rewind file pointer and increment counter */
			rewind(fp);
			fprintf(fp, "%d", (inc + 1));
		}
...
int main(int argc, char* argv[]) {
    ...
    for (i = 0; i < 20; i++) {
		...
		req->inc = getIncarnationNumber(FALSE); /* Assign Incarnation Number */
